 ...
 */
public class PMCLibrary {

	public static final DateFormat DATE_FORMAT = new SimpleDateFormat("yyyy/MM/dd");

	protected static final Logger logger = Logger.getLogger(PMCLibrary.class);
...
//		PropertyConfigurator.configure(path + "/PubMedCrawler2/properties/crawler.cnf");  // modified to the next line!!!
		...
		PMCLibrary library = new PMCLibrary();
	...
	public int batchDownload(Date start_date, Date end_date, boolean force_update) throws IOException {
		...
		PMCRetriever2 retriever = new PMCRetriever2();
	...
	void downloadForDate(Date date, String dateStr, PMCRetriever2 retriever) {
		try {
			...
			failedList.save();
			NcbiPMCEutils.ESearchResult eSearch = retriever.getIdSetByDate(dateStr);
			...
			// saving pmc IDs into files, for each day there is one file containing IDs

			FileWriter fw=new FileWriter("download/PMC_IDs/"+fileName);
	...
	public void crawlFilesByDate(Date date, NcbiPMCEutils.ESearchResult eSearch, PMCRetriever2 retriever) throws Exception {
	...
	private void crawlDateChunk(String file_name, String file_path, NcbiPMCEutils.ESearchResult eSearch, PMCRetriever2 retriever) throws Exception {
